...
# Description
# This script generates a very quick preview of 2s, at 1/4 movie original
# resolution and 1/2 original FPS to check its colors after alterations
# made with UFRaw. It uses the altered frame as reference to check 1s before
# it and 1s after it for the preview. The idea here is to be fast; if you
# want a full-res preview, use the other scripts on this folder.
...
if [ "$((FRAME_POSITION-12))" -gt "0" ]; then
	START_FRAME="$((BASE_FRAME-12))" && END_FRAME="$((BASE_FRAME+12))"; else
	START_FRAME="1" && END_FRAME="51";
fi
...
COUNTER="0";
for ((i="$((FRAME_POSITION-12-1))"; i<="$((FRAME_POSITION+12))"; i="$((i+2))" )) do
...
mplayer "mf://*.jpg" -mf fps=$PREVIEW_FPS:type=jpg -brightness 0 -vf dsize=$PREVIEW_WIDTH:$PREVIEW_HEIGTH;
