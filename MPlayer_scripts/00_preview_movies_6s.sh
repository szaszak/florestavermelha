...
# Description
# This script generates a very quick preview of 6s, at 1/4 movie original
...
if [ "$((FRAME_POSITION-36))" -gt "0" ]; then
	START_FRAME="$((BASE_FRAME-36))" && END_FRAME="$((BASE_FRAME+36))"; else
...
COUNTER="0";
for ((i="$((FRAME_POSITION-36-1))"; i<="$((FRAME_POSITION+36))"; i="$((i+2))" )) do
...
mplayer "mf://*.jpg" -mf fps=$PREVIEW_FPS:type=jpg -brightness 0 -fs #-vf dsize=$PREVIEW_WIDTH:$PREVIEW_HEIGTH;
