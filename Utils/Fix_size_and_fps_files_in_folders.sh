...
# DNGs also have these two reference files in it and opend the program gedit
# to manually correct the files that are missing. The idea is to use it in case
# the script "Check_size_and_fps_files_in_folders.sh" has pointed any errors.
...
for i in `cat empty_size_files.txt`; do
	gedit $i;
	done
	
for j in `cat empty_fps_files.txt`; do
	gedit $j;
	done
